<?php
    ...
    if ($date == ""){
        ...
        if (!$conn){
            ...
            $sql = "SELECT DISTINCT WEEK(salesDate) AS week, YEAR(salesDate) AS year FROM `swe30010`.`Sales` ORDER BY week ASC, year ASC";
        ...
        $md = explode("-", $date);
        $sql = "SELECT * FROM `swe30010`.`Sales` WHERE WEEK(salesDate)=$md[1] AND YEAR(salesDate)=$md[0]";
        $result = mysqli_query($conn, $sql);
