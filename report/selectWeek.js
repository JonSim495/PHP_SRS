...
function exports(item){
    var date = document.getElementById("select-week").value;
    if (item == "item"){
    ...
    // resource: http://halistechnology.com/2015/05/28/use-javascript-to-export-your-data-as-csv/
        ...
        if(week_report.length<=0) return;

        columnDelimiter = week_report.columnDelimiter || ',';
        lineDelimiter = week_report.lineDelimiter || '\n';

        keys = Object.keys(week_report[0]);

        result = '';
        result += keys.join(columnDelimiter);
        result += lineDelimiter;

        week_report.forEach(function(item) {
            ctr = 0;
            keys.forEach(function(key) {
                if (ctr > 0) result += columnDelimiter;
        ...
        console.log(result);
        downloads(result, "Weekly Report " + date + ".csv");
    }
}
...
function populateSelect(arr){
    ...
    options.value = "";
    options.appendChild(document.createTextNode("Select week"));
    options.setAttribute("id", "remove");
    document.getElementById("select-week").appendChild(options);
    ...
    document.getElementById("select-week").onchange = showData;
}
...
function showData(){
    ...
    ajaxToJson(date,
        function(){
            ...
            // fill in week_report
            // TODO: fill in week_report

            // initialise weekly total:
            var weekly = 0;
            ...
            am_td.appendChild(document.createTextNode(weekly));
